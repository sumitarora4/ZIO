...
object Fibers extends ZIOAppDefault {
  ...
  def zibFiberGenerate[E, E1<: E, E2<: E, A, B](fiber1: Fiber[E1, A], fiber2: Fiber[E2, B]): ZIO[Any, Nothing, Fiber[E, (A, B)]] = {
    val finalEffect = for {
      fib1 <- fiber1.join
      fib2 <- fiber2.join
    } yield (fib1, fib2)
  ...
  // spawn n fibers, count the n of words in each file, then aggregate all the results together in one big number

  def generateRandomFile(path: String): Unit = {
    val random = scala.util.Random
    val chars = 'a' to 'z'
    val nwords = random.nextInt(2000) // at least 2000 random words
...
//  def run = zipFiber_v2.debugThread
...
def wordCountParallel(n: Int): UIO[Int] = {
  val effects: Seq[ZIO[Any, Nothing, Int]] = ( 1 to n )
    ...
    .map(path => countWords(path)) // list of effects
